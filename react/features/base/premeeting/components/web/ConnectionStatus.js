...
function ConnectionStatus({ connectionDetails, t, connectionType }: Props) {
    ...
    const onKeyPressToggleDetails = useCallback(
        (e) => {
    ...
    const { connectionClass, icon, connectionText } =
        CONNECTION_TYPE_MAP[connectionType];

    return (
        <div className={classes.connectionStatus}>
            <div aria-level={1} className="con-status-header" role="heading">
